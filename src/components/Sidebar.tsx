...
import { X, Home, Search, Music, Heart, HardDrive, Info, Shield, ExternalLink, User, Smartphone, Star, Share2, Video, BookOpen } from 'lucide-react';
...
export default function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    if (isOpen) hasStoragePermission().then(setStorageGranted);
  }, [isOpen]);
  ...
  const rateApp = () => {
    const url = 'https://play.google.com/store/apps/details?id=com.megan.vibely';
    if (isNativeApp()) Browser.open({ url });
    else window.open(url, '_blank');
  };

  const shareApp = async () => {
    const text = '🎵 Check out Vibely - Global Music Discovery! 1,689 artists, 8,398 songs, 31 countries.\n\nDownload: https://music.megan.qzz.io';
    if (navigator.share) {
      try { await navigator.share({ title: 'Vibely Music', text, url: 'https://music.megan.qzz.io' }); } catch(e) {}
    } else {
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '320px', maxWidth: '85vw', zIndex: 301, background: '#0a0a18', borderLeft: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', overflow: 'auto' }}>
        ...
        <div style={{ padding: '8px' }}>
          {navItems.map((item) => (
            <button key={item.path} onClick={() => handleNav(item.path)} style={{
              ...
              border: 'none', borderRadius: '10px', color: isActive(item.path) ? '#a78bfa' : '#94a3b8',
        ...
        <div style={{ padding: '8px' }}>
          <button onClick={handleStorageRequest} style={{
            ...
            border: 'none', borderRadius: '10px', color: storageGranted ? '#10b981' : '#f59e0b',
            fontSize: '14px', cursor: 'pointer', marginBottom: '2px'
          }}>
            <Shield size={18} />
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontWeight: 600, fontSize: '13px' }}>{storageGranted ? '✅ Storage Granted' : '⚠ Storage Permission'}</div>
              <div style={{ fontSize: '11px', opacity: 0.8 }}>{storageGranted ? 'Music & audio access allowed' : 'Tap to grant music access'}</div>
            </div>
          </button>
        </div>
        ...
        <div style={{ padding: '8px' }}>
          <button onClick={rateApp} style={{
            ...
            borderRadius: '10px', color: '#f59e0b', fontSize: '13px', cursor: 'pointer', marginBottom: '6px'
          }}>
            <Star size={16} /> Rate Vibely ⭐
          </button>
          <button onClick={shareApp} style={{
            display: 'flex', alignItems: 'center', gap: '12px', width: '100%', padding: '12px 16px',
            background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)',
            borderRadius: '10px', color: '#94a3b8', fontSize: '13px', cursor: 'pointer', marginBottom: '6px'
          }}>
            <Share2 size={16} /> Share Vibely
          </button>
        </div>
        ...
        <div style={{ padding: '8px' }}>
          ...
          <a href="https://apps.megan.qzz.io" target="_blank" rel="noopener" style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%', padding: '12px 16px', background: 'rgba(255,255,255,0.02)', border: 'none', borderRadius: '10px', color: '#94a3b8', fontSize: '13px', cursor: 'pointer', marginBottom: '2px', textDecoration: 'none' }}>
            <Smartphone size={16} /> Download Megan Music App
          </a>
        </div>
